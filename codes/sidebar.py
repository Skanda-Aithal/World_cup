...
st.sidebar.image(DATADIR + '/data/images/Logo2.png',use_column_width=True)

def show_x_finals(matches, row, s, f, event_type):
    choosed_match =  st.sidebar.selectbox('Select Match', matches[row][s:f])  
    choosed_match = matches.index[matches[row] == choosed_match][0] 
    home_team = matches['home_team'][choosed_match]
    away_team = matches['away_team'][choosed_match]
    vz.show_match_viz(event_type, home_team, away_team, choosed_match)   

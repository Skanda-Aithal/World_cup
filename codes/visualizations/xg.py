...
def xg_viz(home_team, away_team,event_type):
    ...
    for x in range(len(shots['shot_statsbomb_xg'])):
        ...
        ax.step(x=a_min, y=a_cumulative, where='post', label=away_team)
        ax.step(x=h_min, y=h_cumulative, where='post', label=home_team)
        ax.legend()

def xg_shots(non_goals, goals):
    #Plot all shots using by their xG values.
    
    pitch = VerticalPitch(pitch_type='statsbomb', line_color='#000009', half=True,pad_top=0.5,  
                      pad_bottom=-20,  
                      pad_left=-15,  
                      pad_right=-15,  
                      goal_type='line',
